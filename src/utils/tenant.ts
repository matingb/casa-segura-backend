...
/**
 * Obtiene el id (public.usuario.id) del usuario autenticado buscando por su auth_id (Supabase UID).
 * Lanza un error si el usuario no existe en public.usuario.
 */
export async function getUsuarioIdByAuthId(authId: string): Promise<string> {
  const { rows } = await pool.query(
    'SELECT id FROM public.usuario WHERE auth_id = $1 LIMIT 1',
    [authId]
  );
  if (!rows[0]) {
    throw new Error(`No se encontró el usuario con auth_id: ${authId}`);
